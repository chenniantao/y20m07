...
* @Last Modified time: 2019-11-12 16:05:03
*/
var api = require('api')
var _util = require('util')
var _area = require('util/area')
...
module.exports = {
    ...
    loadProvinces:function(){
        //加载省份列表
        var provinces = _area.getProvinces()
        ...
        //编辑时的处理
        if(this.shipping){
            let provinceCode = this.shipping.areaCode.substr(0,2)+'0000'
            $provinceSelect.val(provinceCode)
            this.loadCities(provinceCode)
        }
    },
    loadCities:function(provinceCode){
        //加载省份对应的城市
        var cities = _area.getCities(provinceCode)
        ...
        //编辑时的处理
        if(this.shipping){
            let cityCode = this.shipping.areaCode.substr(0,4)+'00'
            $citySelect.val(cityCode)
            this.loadCounties(cityCode)
        }        
    },
    loadCounties:function(cityCode){
        //加载城市对应的地区
        var counties = _area.getCounties(cityCode)
        var countiesSelectOptions = this.getSelectOptions(counties)
        var $countySelect = this.$elem.find('.county-select')
        $countySelect.html(countiesSelectOptions)
        //编辑时的处理
        if(this.shipping){
            $countySelect.val(this.shipping.areaCode)
        }        
    },    
    getSelectOptions:function(list){
        var html = '<option value="">请选择</option>'
        for(var code in list){
            html += '<option value="'+code+'">'+list[code]+'</option>'
        }
        return html
    },
    hideModal:function(){
    ...
    bindEvent:function(){
        ...
        //2.城市和地区的联动 
        this.$elem.on('change','.city-select',function(){
            _this.loadCounties($(this).val())
        })        
        //3.提交
    ...
    submit: function() {
        ...
        var formData = {
            ...
            province: _area.getProvinceName($.trim($('[name="province"]').val())),
            city: _area.getCityName($.trim($('[name="city"]').val())),
            county:_area.getCountyName($.trim($('[name="county"]').val())),
            areaCode:$.trim($('[name="county"]').val()),
    ...
    validate: function(formData) {
        ...
        //城市不能为空
        if (!_util.validate(formData.county, 'require')) {
            result.msg = "地区不能为空"
            return result
        }        
        //详细地址不能为空
